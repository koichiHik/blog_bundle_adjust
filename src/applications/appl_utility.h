...
inline void AddRandomNoiseToPoints3d(
    const std::vector<Eigen::Vector3d>& points3d, double mean, double stddev,
...
inline void AddNoise(const SfmData& org_data, SfmData& noised_data) {
  // X. Load extrinsics.
  optimization::AddRandomNoiseToExtrinsic(
      org_data.extrinsic_cams, 0.0, 0.1, 0.0, 0.05, noised_data.extrinsic_cams);
  ...
  */
  ...
  // X. Load points 3d.
  AddRandomNoiseToPoints3d(org_data.points3d, 0.0, 0.1, noised_data.points3d);
  // AddRandomNoiseToPoints3d(org_data.points3d, 0.0, 0.0,
